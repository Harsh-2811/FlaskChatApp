...
client=MongoClient()
chat_db=client.get_database("mydb")
user_collection = chat_db.get_collection("users")
rooms_collection = chat_db.get_collection("rooms")
room_members_collection = chat_db.get_collection("rooms_members")
messages_collection= chat_db.get_collection("messages")

def save_user(username,email,password):
    pass_hash = generate_password_hash(password)
    user_collection.insert_one({"_id":username,"email":email,"password":pass_hash})
...
def get_messages(room_id, page=0):
    ...
    messages = list(
        messages_collection.find({'room_id': room_id}).sort('_id', pymongo.DESCENDING).limit(MESSAGE_FETCH_LIMIT).skip(offset))
